  ...
  try{
    ...
    for (const user of users) {
      ...
      for (const culture_id of user.culture_ids || [])  {
        ...
        const culture = await da.getOneCulture(culture_id)
        const region = await da.getOneRegion(location_id)

        if (!culture || !region) {
          console.warn(`⚠️ Culture ou région non trouvée (culture_id=${culture_id}, location_id=${location_id})`);
          continue; // skip à l’utilisateur suivant
        }

        await da.generateRecommendation({
          user_id: user._id,
          culture_id: culture.name,
    ...
    console.log("Erreur daily worker: ",error)
  }
};
